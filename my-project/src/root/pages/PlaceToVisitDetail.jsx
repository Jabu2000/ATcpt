...
const PlaceToVisitDetail = () => {
    ...
    const loadData = async () => {
      try {
        ...
        const found = saved.find(
          (s) => s.type === "place" && s.refId === data._id
        );
        setIsSaved(!!found);
      } catch (err) {
        console.error(err);
  ...
  const toggleSave = async () => {
    ...
    try {
      if (isSaved) {
        await removeAdventure("place", place._id);
        setIsSaved(false);
        toast.success("Removed from saved");
      } else {
        await saveAdventure("place", place._id, place);
        setIsSaved(true);
  ...
  const submitComment = async () => {
    ...
    try {
      ...
      const res = await fetch(
        `${API_URL}/api/places/${place._id}/comment`,
        {
          method: "POST",
          body: formData,
          credentials: "include",
  ...
  const handleDeleteComment = async (commentId) => {
    try {
      const res = await fetch(`${API_URL}/api/places/${place._id}/comment/${commentId}`, {
        method: "DELETE",
        credentials: "include",
      });
      const data = await res.json();
  ...
  if (!place)
    return <p className="text-center mt-10">Place not found</p>;
  ...
  return (
    <>
      <div className="w-full flex flex-col p-[50px] min-h-screen">
        <Link to="/places" className="text-black text-[20px] mb-4 w-fit">
          <FaArrowLeft />
        </Link>
          ...
          <div className="flex-1 flex flex-col gap-10">
            ...
            <div className="flex flex-row gap-6">
              ...
              {place.openingHours && (
                <div className="w-[60%] bg-[#FF0000] rounded-3xl px-[20px] pt-[40px] pb-[20px]">
                  ...
                  <table className="w-full mt-6 border border-gray-200 rounded-lg overflow-hidden">
                    <tbody>
                      {Object.entries(place.openingHours).map(
                        ([day, hours]) => (
                          <tr
                            key={day}
              ...
              <div className="w-full flex flex-col bg-[#AEFF53] rounded-3xl px-[20px] pt-[40px] pb-[20px] gap-4">
                ...
                {place.phone && (
                  <p className="mt-2 text-black">☎ {place.phone}</p>
                )}
              </div>
            </div>
          </div>
...
export default PlaceToVisitDetail;
