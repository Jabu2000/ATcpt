...
const PlaceToVisitDetail = () => {
  ...
  const submitComment = async () => {
    ...
    try {
      ...
      const res = await fetch(`${API_URL}/api/places/${place._id}/comment`, {
        method: "POST",
        body: formData,
        credentials: "include",
      });
  ...
  const handleDeleteComment = async (commentId) => {
    try {
      const res = await fetch(
        `${API_URL}/api/places/${place._id}/comment/${commentId}`,
        {
          method: "DELETE",
          credentials: "include",
        }
          ...
          <div className="flex-1 flex flex-col gap-10">
            ...
            <div className="flex flex-col lg:flex-row gap-6 mt-6 lg:mt-0">
              ...
              <div className="w-full flex flex-col bg-[#AEFF53] rounded-3xl px-[20px] pt-[40px] pb-[20px] gap-4">
                ...
                {place.about && (
                  <p className="mt-4 text-black font-medium">{place.about}</p>
                )}
                {place.website && (
                  <a
                    href={place.website}
