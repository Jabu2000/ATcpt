...
const API_URL = "https://adventuretimecpt.onrender.com";

/** ⭐ Star Rating Input Component */
const StarRatingInput = ({ value, onChange }) => {
  return (
    <div className="flex gap-1 text-[#FAA500] md:text-[16px] text-[12px] cursor-pointer">
...
const RestaurantDetail = () => {
  ...
  const toggleSave = async () => {
    ...
    try {
      if (isSaved) {
        ...
        await saveAdventure("restaurant", restaurant._id, restaurant);
        setIsSaved(true);
  ...
  const handleDeleteComment = async (commentId) => {
    try {
      const res = await fetch(
        `${API_URL}/api/restaurants/${restaurant._id}/comment/${commentId}`,
        {
          method: "DELETE",
          credentials: "include",
        }
            ...
            <div className="flex flex-wrap md:flex-col-2 mt-[50px] gap-[40px]">
              {restaurant.comments.map((comment) => (
                ...
                >
                  ...
                  <div className="w-full h-[2px] mt-6 bg-black" />
                </div>
              ))}
            </div>
          ) : (
