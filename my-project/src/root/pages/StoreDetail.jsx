...
import StoreCategories from "../../sections/Stores/StoreCategories";
...
const StoreDetail = () => {
    ...
    const loadData = async () => {
      try {
        ...
        const found = saved.find(
          (s) => s.type === "store" && s.refId === data._id
        );
        setIsSaved(!!found);
      } catch (err) {
        console.error(err);
  ...
  const toggleSave = async () => {
    ...
    try {
      if (isSaved) {
        await removeAdventure("store", store._id);
        setIsSaved(false);
        toast.success("Removed from saved");
      } else {
        await saveAdventure("store", store._id, store);
        setIsSaved(true);
  ...
  const submitComment = async () => {
    ...
    try {
      ...
      const res = await fetch(`${API_URL}/api/stores/${store._id}/comment`, {
        method: "POST",
        body: formData,
        credentials: "include",
      });
  ...
  const handleDeleteComment = async (commentId) => {
    try {
      const res = await fetch(
        `${API_URL}/api/stores/${store._id}/comment/${commentId}`,
        {
          method: "DELETE",
          credentials: "include",
        }
  ...
  if (!store) return <p className="text-center mt-10">Store not found</p>;
  ...
  return (
    <>
      <div className="w-full flex flex-col px-6 lg:px-[50px] pt-8 pb-[50px] min-h-screen">
        <Link to="/stores" className="text-black text-[20px] mb-4 w-fit">
          <FaArrowLeft />
        </Link>
