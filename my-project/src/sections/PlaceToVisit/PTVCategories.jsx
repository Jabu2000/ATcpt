...
const PTVCategories = () => {
  ...
  const categories = [
    { key: "NatureOutdoors", title: "Nature out doors", ref: NatureOutdoorsRef },
  ];
  ...
  const fetchPlaces = async (q = "") => {
    try {
      setLoading(true);
      const url = q
        ? `${API_URL}/api/places?q=${encodeURIComponent(q)}`
        : `${API_URL}/api/places`;
  ...
  const filterByCategory = (category) => {
    return places.filter((r) => r.category === category);
  };
  ...
  return (
    <div className="flex flex-col px-6 md:px-[50px] my-[50px]">
      {/* Category Carousels */}
      {categories.map((cat) => {
        const catPlaces = filterByCategory(cat.key);
        return (
          <div key={cat.key} className="flex flex-col 2xl:mt-[150px] mt-[50px]">
            <h2 className="md:text-[30px] text-[20px] text-black font-semibold">{cat.title}</h2>
            {loading ? (
