...
["uploads/comments", "uploads/stores"].forEach((dir) => {
...
const storage = multer.diskStorage({
  destination: (req, file, cb) =>
    cb(
      null,
      file.fieldname === "images" ? "uploads/comments/" : "uploads/stores/"
    ),
...
router.get("/", async (req, res) => {
  ...
  try {
    const stores = await Store.find(filter).sort({ rating: -1, createdAt: -1 });
    res.json(stores);
  } catch (err) {
    console.error(err);
...
router.get("/:id", protect, async (req, res) => {
  ...
  store.openStatus = Store.prototype.getOpenStatus.call(store);

  // ✅ check if user has saved this store
  const isSaved = req.user?.savedItems?.some(
    (id) => id.toString() === store._id.toString()
  );

  res.json({ ...store.toObject(), isSaved });
});

router.post("/", upload.array("images", 5), async (req, res) => {
  const newStore = new Store({
    ...req.body,
    images: req.files?.map((f) => `/uploads/stores/${f.filename}`) || [],
  });
  await newStore.save();
  res.json(newStore);
});
...
router.put("/:id", upload.single("image"), async (req, res) => {
  ...
  if (req.file) {
    ...
    updateData.images.push(`/uploads/stores/${req.file.filename}`);
  }
